...
- End: The blank line before the next class or section (likely before JsonCompareFormatter)

This includes all methods:
- __init__
- format_header
- format_match_group
- format_group_lines (shared helper - check if used by TextActionFormatter first!)
- format_unmatched
- format_unmatched_header
- format_summary
- format_statistics
...
**IMPORTANT:** Check if `format_group_lines` is used by TextActionFormatter. If so, move it to TextActionFormatter or make it a standalone function before deleting TextCompareFormatter. This was added in quick-002 as a shared helper.

Review the implementation to identify any shared code that needs to be preserved.
  </action>
  <verify>Run: `grep -n "class TextCompareFormatter" file_matcher.py` - should return no results</verify>
  <done>TextCompareFormatter class deleted; any shared utilities preserved</done>
</task>
