...
Note: This plan depends on 04-01 and 04-02. When executing, the functions from those plans
(execute_action, execute_all_actions, create_audit_logger, log_operation, etc.) will already
exist in file_matcher.py. Reference those functions directly in the codebase, not summary files.
</context>
...
class TestActionExecution(BaseFileMatcherTest):
    ...
    def test_partial_failure_returns_exit_code_3(self):
        ...
        # Set up: Create duplicates, then make one duplicate's parent read-only
        # to cause permission error on that file only

        # For simpler testing, mock execute_action to fail for specific files
        original_execute_action = None
        call_count = [0]
        ...
        # Note: If no duplicates exist, this may return 0. Ensure test fixture has duplicates.
        self.assertIn(exit_code, [0, 3])  # 0 if no dups to process, 3 if partial failure
...
- test_partial_failure_returns_exit_code_3 implemented with mocked failures
  </done>
</task>
