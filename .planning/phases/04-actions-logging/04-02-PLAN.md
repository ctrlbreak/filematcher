...
```bash
# Test 1: --log with --execute should work (shows preview, then would execute)
python file_matcher.py test_dir1 test_dir2 --master test_dir1 --action hardlink --execute --yes --log test.log
# Should execute without error about unrecognized argument

# Test 2: --log without --execute should error
...
# Cleanup
rm -f test.log filematcher_*.log
```
  </verify>
  <done>
