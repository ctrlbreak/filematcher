...
must_haves:
  ...
  artifacts:
    ...
    - path: "CHANGELOG.md"
      ...
      contains: "JSON output schema v2.0"
  key_links:
...
Output: Updated JsonActionFormatter that produces header-based JSON structure, updated test suite validating the new schema, and breaking change documentation.
</objective>
...
<task type="auto">
  <name>Task 3: Document breaking change in release notes</name>
  <files>CHANGELOG.md</files>
  <action>
Add a breaking change entry for the JSON schema restructuring:

1. If CHANGELOG.md exists, add entry under the "Unreleased" or next version section
2. If CHANGELOG.md doesn't exist, create it with standard format

Entry should include:
```markdown
### Breaking Changes

- **JSON output schema v2.0**: Restructured JSON output with unified header object
  - All metadata (timestamp, mode, action, hashAlgorithm, directories) moved to `header` object
  - Directory keys renamed: `dir1`/`dir2` -> `master`/`duplicate`
  - Unmatched arrays renamed: `unmatchedDir1`/`unmatchedDir2` -> `unmatchedMaster`/`unmatchedDuplicate`
  - Header includes `name` ("filematcher") and `version` ("2.0") fields for schema identification
  - Migration: Update JSON parsers to access metadata via `data.header.*` instead of `data.*`
```

This documents the breaking change so consumers know to update their parsers.
  </action>
  <verify>
```bash
grep -q "JSON output schema v2.0" CHANGELOG.md && echo "Breaking change documented: OK"
```
  </verify>
  <done>
Breaking change documented in CHANGELOG.md with migration guidance for JSON consumers.
  </done>
</task>
...
3. Manual verification of JSON output structure:
   ...
   ```
4. Verify CHANGELOG.md contains breaking change documentation
</verification>
...
8. Breaking change documented in CHANGELOG.md
</success_criteria>
