...
must_haves:
  truths:
    ...
    - "All 39 JSON tests pass with new structure"
  artifacts:
...
- Maintain existing sorting for determinism
  ...
  <verify>
Run targeted test that validates JSON structure:
```bash
python3 -c "
...
```
  ...
  <done>
JsonActionFormatter produces JSON with header object containing name, version, timestamp, mode, hashAlgorithm, directories. Compare mode uses master/duplicate keys. Action modes include action field in header.
  </done>
</task>
...
```bash
python3 -m pytest tests/test_json_output.py -v
```
...
<verification>
1. Run full JSON test suite: `python3 -m pytest tests/test_json_output.py -v` - all 39+ tests pass
...
6. All JSON tests pass (39+ tests)
